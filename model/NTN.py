...
class NTN:
    ...
    def _compute_y_pred_for_node_derivative(self, env, target_tensor, inputs, y_true):
        """
        Compute y_pred for derivative calculation.
        Override this in subclasses to use ensemble predictions.
        """
        return (env & target_tensor).contract(output_inds=[self.batch_dim] + self.output_dimensions)
    ...
    def _batch_node_derivatives(self, inputs, y_true, node_tag):
        ...
        y_pred = self.forward_from_environment(env, node_tag=node_tag, node_tensor=target_tensor)
