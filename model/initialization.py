...
def normalize_tn_output(tn, input_samples, output_dims, batch_dim='s', 
                       target_std=0.1, max_samples=1000):
    """
...
def normalize_tn_frobenius(tn, target_norm=1.0, exclude_tags=None):
    """
...
def init_mps_normalized(L, bond_dim, phys_dim, output_dim, output_site=None,
                       base_init=0.1, sample_inputs=None, target_std=0.1):
    """
    ...
    model = MPO2(L=L, bond_dim=bond_dim, phys_dim=phys_dim, 
                output_dim=output_dim, output_site=output_site,
                init_strength=base_init)
    
    if sample_inputs is not None:
        scale = normalize_tn_output(
            model.tn, sample_inputs, 
...
def init_mps_orthogonal(L, bond_dim, phys_dim, output_dim, output_site=None,
                       init_strength=0.1, method='qr'):
    """
    ...
    if method == 'quimb':
        ...
        if len(original_shape) == 3:
            ...
            temp_shape = (original_shape[0], original_shape[1], original_shape[2] * original_shape[3])
        ...
        for i, tensor in enumerate(model.tn):
            ...
            if torch.is_tensor(matrix):
                ...
                if q.numel() == np.prod(original_shape):
                    ...
                    scaled_matrix = matrix * init_strength / (matrix.norm() / np.sqrt(matrix.numel()))
                ...
                if q.size == np.prod(original_shape):
                    ...
                    scaled_matrix = matrix * init_strength / (np.linalg.norm(matrix) / np.sqrt(matrix.size))
                    orthogonal_data = scaled_matrix.reshape(original_shape)
