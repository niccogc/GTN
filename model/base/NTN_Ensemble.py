...
class NTN_Ensemble:
    ...
    def __init__(
        ...
        # Memory optimization: Ensure all Inputs objects reference the SAME underlying torch tensors.
        # We create input tensors ONCE and reuse the same list objects to guarantee no duplication
        # when tensors are on CUDA. CUDA cache is also cleared after heavy operations.

        # Create input tensors ONCE - these are the only copies that should exist
        # X_train_with_bias is a new tensor (necessary for bias column)
        # X_train is reused directly (no copy) for all other models
        X_train_with_bias = torch.cat(
            [
                X_train,
                torch.ones(X_train.shape[0], 1, dtype=X_train.dtype, device=X_train.device),
            ],
            dim=1,
        )

        # Create SHARED input/output lists ONCE - reuse these exact list objects
        # This ensures all Inputs objects reference the same underlying tensors
        shared_inputs_with_bias = [X_train_with_bias]
        shared_inputs_no_bias = [X_train]  # Direct reference to original tensor
        shared_outputs_train = [y_train]

        if X_val is not None and y_val is not None:
            X_val_with_bias = torch.cat(
                [
                    X_val,
                    torch.ones(X_val.shape[0], 1, dtype=X_val.dtype, device=X_val.device),
                ],
                dim=1,
            )
            shared_val_inputs_with_bias = [X_val_with_bias]
            shared_val_inputs_no_bias = [X_val]  # Direct reference to original tensor
            shared_outputs_val = [y_val]
        else:
            shared_val_inputs_with_bias = None
            shared_val_inputs_no_bias = None
            shared_outputs_val = None

        # Cache for shared Inputs loaders (keyed by input_labels tuple)
        ...
        for idx, (tn, input_dims, input_labels) in enumerate(
            ...
            input_labels_key = tuple(input_labels)

            # First model (idx=0) gets bias appended - always unique
            if idx == 0:
                train_loader = Inputs(
                    inputs=shared_inputs_with_bias,  # Reuse same list object
                    outputs=shared_outputs_train,    # Reuse same list object
                ...
                # For idx > 0, reuse shared loader if input_labels match
                if input_labels_key not in shared_train_loaders:
                    shared_train_loaders[input_labels_key] = Inputs(
                        inputs=shared_inputs_no_bias,  # Reuse same list object (same X_train tensor)
                        outputs=shared_outputs_train,  # Reuse same list object
                        outputs_labels=output_dims,
                        input_labels=input_labels,
                        batch_dim=self.batch_dim,
                ...
                train_loader = shared_train_loaders[input_labels_key]
            ...
            if X_val is not None and y_val is not None:
                if idx == 0:
                    val_loader = Inputs(
                        inputs=shared_val_inputs_with_bias,  # Reuse same list object
                        outputs=shared_outputs_val,          # Reuse same list object
                        outputs_labels=output_dims,
                        input_labels=input_labels,
                        batch_dim=self.batch_dim,
                        batch_size=batch_size,
                    ...
                    # For idx > 0, reuse shared loader if input_labels match
                    if input_labels_key not in shared_val_loaders:
                        shared_val_loaders[input_labels_key] = Inputs(
                            inputs=shared_val_inputs_no_bias,  # Reuse same list object (same X_val tensor)
                            outputs=shared_outputs_val,        # Reuse same list object
                            outputs_labels=output_dims,
                            input_labels=input_labels,
                            batch_dim=self.batch_dim,
            ...
            for batch_idx in range(n_batches):
                ...
                for ntn_idx, other_ntn in enumerate(self.ntns):
                    ...
                    if total is None:
                        ...
                        total = total + y_pred
                        del y_pred  # Free intermediate tensor
