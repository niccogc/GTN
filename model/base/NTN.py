...
class NTN:
    ...
    def _batch_environment(
        ...
        # Get target node's indices BEFORE deleting (needed for CPDA-style networks)
        ...
        # Compute env output_inds using unified formula:
        # env_inds = {batch_dim} ∪ node_inds ∪ out_labels - (node_inds ∩ out_labels)
        # This works for both MPS-style (bond indices) and CPDA-style (shared rank index)
        intersection = node_inds & out_labels
        env_inds = ({self.batch_dim} | node_inds | out_labels) - intersection
    ...
    def fit(
        ...
        For regression (MSELoss, MAELoss, HuberLoss): uses direct least squares solver.
        For classification (CrossEntropyLoss): uses Newton-based optimization.
