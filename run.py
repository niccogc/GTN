...
def run_gtn(cfg: DictConfig, model, data: dict, output_dir: Path) -> dict:
    ...
    X_val = torch.cat(
        [
            data["X_val"],
            torch.ones(
                data["X_val"].shape[0], 1, dtype=data["X_val"].dtype, device=data["X_val"].device
            ),
        ],
        dim=1,
    )
    ...
    train_loader = torch.utils.data.DataLoader(
        torch.utils.data.TensorDataset(X_train, data["y_train"]),
        batch_size=batch_size,
        shuffle=True,
    )
    val_loader = torch.utils.data.DataLoader(
        torch.utils.data.TensorDataset(X_val, data["y_val"]),
        batch_size=batch_size,
        shuffle=False,
    )
