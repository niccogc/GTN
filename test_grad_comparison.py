...
Both models use the same architecture and data for fair comparison.
"""
import torch
...
print("="*70)
print("GTN vs NTN Comparison on MNIST")
print("="*70)
...
SUBSET_SIZE = 5000  # Use 5k samples instead of 60k
TEST_SIZE = 1000
...
# For NTN with 3D MNIST data (samples, patches, pixels), we use explicit list format
# to have multiple input labels point to the same source data
# Format: [source_idx, (indices...)]
input_labels_ntn = [
    [0, ("0_patches", "0_pixels")],  # All three point to source 0 (xinp_train)
...
loader_ntn = Inputs(
    ...
    input_labels=input_labels_ntn,  # Use explicit format for 3D data
    batch_dim="s",
...
model_ntn = NTN(
    ...
    input_dims=input_labels,  # Keep original for NTN.input_dims
    loss=loss_ntn,
...
def evaluate_ntn():
    """Evaluate NTN on test set"""
    # Forward pass
    output_tn = model_ntn.tn.copy()
    for inp in test_inputs_ntn:
        output_tn = output_tn & inp
    
    result = output_tn.contract(output_inds=["s", "class_out"], optimize='auto-hq')
    logits = result.data
